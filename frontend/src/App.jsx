...
function App() {
	function OperatingHoursTable() {
		...
		return (
			<>
			<table className="text-xl md:text-4xl font-serif text-gray-500 m-5">
				<tbody>
					{hours.map((row, index) => (
						<tr key={index}>
  ...
  return (
    <>
		...
		<div className="w-full min-h-screen flex flex-col bg-gray-500 overflow-x-hidden">
		  <div className="bg-fixed justify-center w-full flex flex-col items-center bg-cover bg-center min-h-[100vh]" style={{ backgroundImage: "url('/gelato.jpg')"}}>
			<div className="backdrop-blur-lg bg-[#e4d5b7]/50 rounded-3xl shadow-xl/20 items-center text-center m-10">
				<h1 className="text-5xl md:text-7xl lg:text-8xl font-bold text-gray-700 font-serif p-5">PRONTO CAFE</h1>
		  	</div>
		  </div>
