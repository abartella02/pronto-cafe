				...
				<table className="text-xl sm:text-2xl lg:text-4xl font-serif text-gray-700 mt-2 mb-3">
					<tbody>
						{Array.from({ length: midpoint }).map((_, index) => (
							<tr key={index}>
								<td className="text-left py-1 pr-5 md:pr-20">
									{col1[index] &&
										(col1[index]?.vegan
											? `${col1[index]?.name.toProperCase()} (V)`
											: col1[index]?.name.toProperCase())}
								</td>
								<td className="text-left py-1">
									{col2[index] &&
										(col2[index]?.vegan
											? `${col2[index]?.name.toProperCase()} (V)`
											: col2[index]?.name.toProperCase())}
								</td>
							</tr>
						))}
					</tbody>
			...
			<>
				<table className="width-full text-xl md:text-4xl font-serif text-gray-500 m-5">
					<tbody>
						{sizes.map((row, index) => (
							<tr key={index}>
								<td className="text-left py-2 pr-20 md:pr-50">
									{row.name.toProperCase()}
								</td>
			...
			<div>
				...
				<div className="w-full p-8 pb-12 flex flex-col bg-[#f4f1de] items-center text-center">
					...
					<p className="font-serif text-md sm:text-lg lg:text-xl md:pb-5">
						Please be advised that all our flavours contain traces
						of&nbsp;
						<span className="font-bold font-serif">
							peanuts, tree nuts, and gluten.{" "}
						</span>
						Thank you for your cooperation.
					</p>
					<div>
						<FlavoursTable />
						<p className="text-lg sm:text-xl lg:text-3xl font-serif text-gray-700">
							V = Vegan
						</p>
					</div>
				</div>
				...
				<div className="w-full flex flex-col items-center bg-gray-200 p-10">
					...
					<table className="text-xl md:text-3xl text-gray-800 font-serif m-2 md:m-5">
						<tbody>
							<tr>
								<td className="text-left pr-4 pb-3">
							...
							<tr>
								...
								<td className="text-left pb-3">
									<a
										className="underline"
										href="https://www.instagram.com/_prontocafe/"
									>
										@_prontocafe
									</a>
								</td>
							...
							<tr>
								...
								<td className="text-left pb-3">
									9222 Keele Street, Vaughan, ON L4k 5A3
								</td>
							</tr>
						</tbody>
					</table>
